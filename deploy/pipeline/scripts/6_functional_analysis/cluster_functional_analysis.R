...
go_overrep <- function(net_heats, back_nodes, name, id,folder){
  ...
  if (nrow(go1) > 0) {
    ...
    # Save dot plot
    filep <- file.path(folder, paste0(name, "_go_overrep_dot.pdf"))
    pdf(filep)
    print(dot_plot)
    dev.off()
...
if (nrow(clusters) >0) {
  ...
  for (i in cluster_dfs){
    ...
    if (go_res != ""){
      write.table(go_res, file = file.path(path, paste0(nam, "_go_overrep_results.txt")), quote = FALSE, row.names = FALSE, sep = "\t")
    }
  ...
  # Run together
  clusters_go <- compareCluster(compare_ids, fun="enrichGO", ont = 'BP', OrgDb='org.Hs.eg.db',qvalueCutoff = 0.05, universe = back_nodes_e$ENTREZID)
  #clusters_go2 <- simplify(clusters_go,cutoff = 0.7,by = "p.adjust")
  if(nrow(clusters_go) > 0){
    dotplot_g <- dotplot(clusters_go, font.size = 10, title = paste0("Clustering of PPI nodes"), showCategory=5)
  ...
  clusters_react <- compareCluster(compare_ids, fun="enrichPathway",qvalueCutoff=0.05, universe =back_nodes_e$ENTREZID)
  #summary(clusters_react)
  if(nrow(clusters_react) > 0){
    dotplot_r <- dotplot(clusters_react, font.size = 10, title = paste0("Clustering of PPI nodes"), showCategory=5)
